...
class AntColonyOptimizer:
    ...
    def fit(self, map_matrix, iterations=100, mode='min', early_stopping_count=20, verbose=True):
        ...
        if mode == 'min':
            ...
            return self.best, best_path_coords, best_path, best_score
        elif mode == 'max':
...
def main():
    ...
    for r in range(NUM_CITIES):
        for c in range(NUM_CITIES):
            cost_matrix[r, c] = problem.distance(r, c)
    ...
    _, _, best_path, _ = optimizer.fit(cost_matrix, 400, verbose=False)
    #optimizer.plot()
    problem.plot(np.array(best_path))
